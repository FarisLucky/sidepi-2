      ...
      <div class="col-sm-12">
        <div class="card">
          <div class="card-body p-4">
            <div class="row">
              <div class="col-sm-12">
                <a href="<?= base_url('pengeluaran') ?>" class="btn btn-dark float-right"><i class="fa fa-arrow-circle-left"></i> Kembali</a>
              </div>
            </div>
            <div class="row">
              <div class="col-sm-12">
                <form action="<?php echo base_url() . 'pengeluaran/coreubah'; ?>" method="post" enctype="multipart/form-data">
                  <input type="hidden" name="<?= $this->security->get_csrf_token_name(); ?>" value="<?= $this->security->get_csrf_hash(); ?>">
                  <input type="hidden" name="params" value="<?php echo $p->id_pengeluaran ?>">
                  <div class="form-group">
                    <label>Pengeluaran</label>
                    <input type="text" name="nama_pengeluaran" value="<?php echo $p->nama_pengeluaran ?>" class="form-control">
                    ...
                    <select name="unit" class="form-control select-opt">
                      ...
                      <?php foreach ($unit as $u) { ?>
                        <option value=<?= '"' . $u->id_unit . '"';
                                        echo ($u->id_unit == $p->id_unit) ? 'selected' : ''; ?>>
                          <?= $u->nama_unit ?></option>
                      <?php } ?>
                    </select>
                  </div>
                  <div class="form-group">
                    ...
                    <select name="kelompok" class="form-control select-opt">
                      ...
                      <?php foreach ($kelompok as $key => $value) { ?>
                        <option value=<?= '"' . $value->id_kelompok . '"';
                                        echo ($value->id_kelompok == $p->id_kelompok) ? "selected" : ""; ?>>
                  ...
                  <div class="form-group">
                    ...
                    <?php if (!empty($p->bukti_kwitansi)) { ?>
                      <img src="<?= base_url('assets/uploads/images/pengeluaran/' . $p->bukti_kwitansi) ?>" id="tampil_foto" class="img-thumbnail">
                    <?php } else { ?>
                      <img id="tampil_foto" class="img-thumbnail">
                      <h4 class="p-4">Gambar Kosong</h4>
                    <?php }; ?>
                    <input type="file" name="bukti_kwitansi" class="form-control" onchange="validateFileUpload(this);readURL(this,'#tampil_foto')">
                  </div>
